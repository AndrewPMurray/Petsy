...
const SingleReview = forwardRef(({ review, seller, products, pageNum }, ref) => {
	...
	}, [products?.length, pageNum, contentRef.current?.scrollHeight, review?.content]);
	...
	return reviewLength === 0 ? (
		<div ref={ref} className='single-review-container-div'></div>
	) : (
			...
			<div className={`review-left-side-${photoPresent}`}>
				...
				{tooLong ? (
					<div className='review-content-div overflow-div'>
						...
						<button className='review-ellipsis-button' onClick={handleExpandContent}>
							•••
						</button>
					</div>
				) : (
					...
					<div className='purchased-item-div'>
						...
						<div className='seller-review-product-info'>
							<img
								...
								alt='tiny-seller-review'
							></img>
							<ScrollToTop>
								<button
									className='button-to-other-product'
